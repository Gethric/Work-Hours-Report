...
export const addItem = (sched = 'dfsched', lvl = 'dflvl', teacher = 'dfteacher', hrs = 'dfhrs', comp = 'dfcomp', freq = 'dffreq', chat = 'dfchat', date = 'dfdate', kt = 'dfkt', subabtime = sbabdf) => {
    ...
    if (sched !== 'Absences' && sched !== 'Companies' && sched !== 'Substitutions' && sched !== 'Chat') {
        type = 'Courses';
    }
    ...
    if (type === 'Absences') {
        newItem = new Absence(ID, type, sched, lvl, teacher, hrs, comp, freq, chat, date, kt, subabtime);

    } else if (type === 'Companies') {
        newItem = new Companies(ID, type, sched, lvl, teacher, hrs, comp, freq, chat, date, kt, subabtime);

    } else if (type === 'Substitutions') {
        newItem = new Substitution(ID, type, sched, lvl, teacher, hrs, comp, freq, chat, date, kt, subabtime);

    } else if (type === 'Chat') {
        newItem = new Chat(ID, type, sched, lvl, teacher, hrs, comp, freq, chat, date, kt, subabtime);

    } else {
        newItem = new Course(ID, type, sched, lvl, teacher, hrs, comp, freq, chat, date, kt, subabtime);
    }
